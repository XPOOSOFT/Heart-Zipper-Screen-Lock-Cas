...
//            }
        ...
        locker?.addOnTouchMoveListner { urect, _, d2 ->
            if (ClickDown && d2 > urect.Height() / 2.0) {
                LastY = urect.top
                if (!PasswordCorrect && ClickPassword && (d2 - urect.Height() / 1.6) >= Screen.Height * 0.8) {
//                    Height = Screen.Height * 0.8
                    SllideDownBottom()
                } else {
                    if ((d2 - urect.Height() / 1.6) >= Screen.Height * 0.8) {
//                    Height = Screen.Height * 0.8
                        SlideDownAfterTime(1)
                    } else {
                        if (ClickPassword && (d2 - urect.Height() / 1.6) >= Screen.Height * 0.8) {
                            SllideDownBottom()
                        }
                    }
                }
                urect.top = (d2 - urect.Height() / 1.6)
            }
        }
        locker?.OnUpdateListner { urect ->
            when (unlocktype) {
...
//        val d = Screen.Width
    ...
    fun SlideDownAfterTime(i: Int) {
        try {// Validate the input value 'i'
            if (i < 0) {
                return
            }

            // Use Timer safely
            Timer(i, 0).OnTimerFinishCounting {
                SllideDown()  // Ensure this method exists and is correct
            }
            Timer(i, 0).start()
        } catch (e: Exception) {
            e.printStackTrace()
...
//                i = AppConfig.AnimationSpeed
                ...
                deplace =
                    Deplace(
                        ...
                        locker?.left ?: return,
                        (Screen.Height * 2.0),
                        (AppConfig.AnimationSpeed * 2).toDouble(),
                        Transition_Type.easeOutQuad,
                        0.0
                    )
            }
    ...
    fun UpdateType3(d: Double) {
        ...
        for (i3 in actionLayer.left?.indices ?: return) {
            ...
            if (d > uimagePart.top) {
                ...
                uimagePart.setWidth(GetValue)
                uimagePart.ImageRect?.setWidth(
                    uimagePart.Width() / Screen.Width * (uimagePart.image?.width?.toDouble()
                        ?: return)
                )
                val uimage = uimagePart.childrens?.get(0) as Uimage
                uimage.left =
